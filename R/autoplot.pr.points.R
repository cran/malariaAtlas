...
if(plot_species_n == 1){
  pr_plot <- pr_plot +
    ...
    ggplot2::geom_point(data = object[!object$species %in% "Confidential",], aes(.data$longitude, .data$latitude, fill = .data$pr, size = .data$examined), alpha = 0.8, shape = 21, na.rm = TRUE)+
  ...
  if("Confidential" %in% unique(object$species)){
    pr_plot$layers <- c(pr_plot$layers[[1]], geom_point(data = object[object$species == "Confidential",],
                 aes(.data$longitude, .data$latitude),
